...
func (r *StatisticsRepository) GetTasks(ctx context.Context, userID *int, from, to *time.Time) ([]domain.Task, error) {
	...
	queryBuilder.WriteString(`SELECT id, title, description, completed, created_at, completed_at, author_user_id FROM todoapp.tasks;`)
	...
	if from != nil {
		conditions = append(conditions, fmt.Sprintf("create_at>=$%d", len(args)+1))
	...
	queryBuilder.WriteString(" ORDERED BY ASC ")
